       ...
       try {
        ...
        if(pageNumber === 1){
            setProduct(paginateData)
              ...
              setProduct(prev=> [...prev, ...paginateData])
        }
  ...
  return (
    <SafeAreaView className='flex-1 bg-surface' edges={['top']}>
       ...
       {loading ? (
        ...
        <FlatList data={product} keyExtractor={(item)=>item._id} numColumns={2} contentContainerStyle={{padding: 16, paddingBottom: 100}} columnWrapperStyle={{justifyContent: 'space-between'}} renderItem={({item})=>(
